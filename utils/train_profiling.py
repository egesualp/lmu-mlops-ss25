...
# Optional: restrict profiling to a few steps

accuracy_metric = evaluate.load("accuracy")

class TrainerWithProfiler(Trainer):
    def __init__(self, *args, **kwargs):
        self.enable_profiling = kwargs.pop("enable_profiling", False)
        self.profiler = None
        self.profile_steps = kwargs.pop("profile_steps", 3)
        super().__init__(*args, **kwargs)

    def training_step(self, model, inputs, num_steps):
        if self.enable_profiling and self.profiler is None:
            self.profiler = profile(
                activities=[ProfilerActivity.CPU],
                schedule=torch.profiler.schedule(wait=1, warmup=1, active=self.profile_steps, repeat=1),
                on_trace_ready=tensorboard_trace_handler("logs/train_profiling"),
                #record_shapes=True,
                #with_stack=True
            )
...
def train(cfg: DictConfig):
    ...
    enable_profiling = cfg.get("enable_profiling", False)
    max_profile_steps = cfg.get("max_profile_steps", 3)
    ...
    trainer = TrainerWithProfiler(
        ...
        profile_steps=max_profile_steps
    )
    ...
    with profile(
    activities=[ProfilerActivity.CPU],
    schedule=torch.profiler.schedule(wait=1, warmup=1, active=max_profile_steps, repeat=1),
    on_trace_ready=tensorboard_trace_handler("log/bert_profiling"),
    #record_shapes=True,
    #with_stack=True
    ) as prof:
        for step in range(max_profile_steps + 2):  # +2 for wait/warmup
            trainer.train()
            prof.step()
...
class TrainerWithProfiler(Trainer):
    def __init__(self, *args, **kwargs):
        self.enable_profiling = kwargs.pop("enable_profiling", False)
        self.profiler = None
        self.profile_steps = kwargs.pop("profile_steps", 5)
        super().__init__(*args, **kwargs)
    ...
    def training_step(self, model, inputs):
        ...
        # regular HuggingFace training step
